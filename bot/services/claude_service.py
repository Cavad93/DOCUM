...
Заполните шаблон:"""
    ...
    async def correct_template(
        ...
        try:
            ...
            s = self._sanitize
            prompt = f"""Вы медицинский ассистент. Вам нужно ИЗМЕНИТЬ готовый шаблон осмотра согласно комментариям пользователя.

╔══════════════════════════════════════════════════════╗
║  КОММЕНТАРИИ ПОЛЬЗОВАТЕЛЯ — ГЛАВНЫЙ ПРИОРИТЕТ!       ║
║  Применить КАЖДЫЙ пункт. Ничего не пропустить.       ║
╚══════════════════════════════════════════════════════╝

{s(corrections, max_len=1000)}

КРИТИЧЕСКИ ВАЖНО:
- Внесите ВСЕ изменения из комментариев выше — без исключений
- Если пользователь просит добавить — ДОБАВЬТЕ в нужный раздел
- Если пользователь просит убрать — УБЕРИТЕ
- Если пользователь просит изменить — ИЗМЕНИТЕ
- НЕ возвращайте исходный документ без изменений!
- НЕ игнорируйте ни один пункт из комментариев!

ТЕКУЩИЙ ШАБЛОН ДЛЯ ИЗМЕНЕНИЯ:
{current_template}

ЗАФИКСИРОВАННЫЕ ДАННЫЕ (не менять без команды пользователя):
- ФИО: {s(patient_data.full_name)}
- Дата рождения: {s(patient_data.birth_date)}
{snils_line}{workplace_line}{position_line}- Диагноз: {s(patient_data.diagnosis)}

ВАЖНЫЕ ДАТЫ (не менять):
{illness_line}{call_line}{exam_date_line}{eln_line}{follow_up_line}

ДОПОЛНИТЕЛЬНЫЕ ИНСТРУКЦИИ:
3. {self._get_workplace_instruction(patient_data)}
4. {self._get_position_instruction(patient_data)}
5. Даты осмотра/болезни/вызова НЕ менять (см. выше)
6. Структуру разделов сохранить
7. НЕ добавляйте текст до или после шаблона
8. Верните ТОЛЬКО изменённый шаблон
...
КРИТИЧЕСКИ ВАЖНО - НЕ ВКЛЮЧАЙТЕ В ОТВЕТ:
1. НЕ включайте шапку документа (логотип, контакты клиники)
2. НЕ включайте главный заголовок ("Осмотр терапевта на дому" или любой другой)
3. Возвращайте только СОДЕРЖИМОЕ осмотра (без шапки и заголовка)

ПРАВИЛА ФОРМАТИРОВАНИЯ ТЕКСТА:
1. Используйте **двойные звёздочки** для ЖИРНОГО текста
2. Обычный текст пишите БЕЗ звёздочек
3. Нумерованные списки: "1. Текст", "2. Текст" и т.д.
4. Заголовки разделов ВСЕГДА жирные: **Заголовок:**
5. "План лечения" и "План обследования" — ВСЕГДА нумерованные списки
